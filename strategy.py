...
class StressTestStrategy(Strategy):
    ...
    def calculate_signals(self, event):
        ...
        if self.bar_count == 1:
            logger.info(f"DAY {self.bar_count}: Initial MKT BUY (10 shares @ {current_price})")
            signal = SignalEvent(1, self.symbol, event.timeindex, 'LONG', 1.0,
                                 sizing_type='FIXED_SHARES', sizing_value=10,
                                 order_type='MKT')
            self.events.put(signal)

        # 2. Day 2: Add to LONG with Market Order (% Equity)
        elif self.bar_count == 2:
            logger.info(f"DAY {self.bar_count}: Add to LONG with MKT BUY (10% of equity @ {current_price})")
            signal = SignalEvent(1, self.symbol, event.timeindex, 'LONG', 1.0,
                                 sizing_type='PERCENT_EQUITY', sizing_value=0.10,
                                 order_type='MKT')
            self.events.put(signal)
        ...
        # 3. Day 3: Place a LIMIT order to buy on a dip
            ...
            limit_price = 95.0
            logger.info(f"DAY {self.bar_count}: Placing LMT BUY order (5 shares @ {limit_price})")
            signal = SignalEvent(1, self.symbol, event.timeindex, 'LONG', 1.0,
                                 sizing_type='FIXED_SHARES', sizing_value=5,
                                 order_type='LMT', limit_price=limit_price)
            self.events.put(signal)
        ...
        # 4. Day 4: Place a STOP order to sell on a price drop (Stop-Loss)
            ...
            stop_price = 96.0
            logger.info(f"DAY {self.bar_count}: Placing STP SELL order (all shares @ {stop_price})")
            signal = SignalEvent(1, self.symbol, event.timeindex, 'EXIT', 1.0,
                                 sizing_type='FIXED_SHARES', sizing_value=self.portfolio.current_positions[self.symbol],
                                 order_type='STP', stop_price=stop_price)
            self.events.put(signal)
        ...
        # 5. Day 5: Flip position to SHORT
            ...
            logger.info(f"DAY {self.bar_count}: Flipping to SHORT with MKT SELL (30 shares @ {current_price})")
            signal = SignalEvent(1, self.symbol, event.timeindex, 'SHORT', 1.0,
                                 sizing_type='FIXED_SHARES', sizing_value=30,
                                 order_type='MKT')
            self.events.put(signal)
        ...
        # 6. Day 6: Place a TRAIL BUY order to protect short position
            ...
            if self.portfolio.current_positions[self.symbol] < 0:
                ...
                logger.info(f"DAY {self.bar_count}: Placing TRAIL BUY order (all shares with ${trail_offset} trail)")
                signal = SignalEvent(1, self.symbol, event.timeindex, 'EXIT', 1.0,
                                     sizing_type='FIXED_SHARES', sizing_value=abs(self.portfolio.current_positions[self.symbol]),
                                     order_type='TRAIL', trail_price=trail_offset)
                self.events.put(signal)

        # 7. Day 8: Use an IMMEDIATE FILL order
        elif self.bar_count == 8:
            logger.info(f"DAY {self.bar_count}: Placing Immediate Fill MKT BUY (5 shares @ {current_price})")
            signal = SignalEvent(1, self.symbol, event.timeindex, 'LONG', 1.0,
                                 sizing_type='FIXED_SHARES', sizing_value=5,
                                 order_type='MKT', immediate_fill=True)
            self.events.put(signal)
